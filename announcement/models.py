...
class Announcement(BaseModel):
    ...
    active_start_time = models.DateTimeField(
        blank=True, null=True, verbose_name=_("生效時間")
    )
    active_end_time = models.DateTimeField(
        blank=True, null=True, verbose_name=_("失效時間")
    )
    active_member = models.ManyToManyField(User, blank=True, verbose_name=_("發布人"))
    is_send_email = models.BooleanField(
        default=False, verbose_name=_("是否發送電子郵件通知")
    )
    email_sent_at = models.DateTimeField(
        blank=True,
        null=True,
        verbose_name=_("Email 發送時間"),
        help_text=_("記錄 email 發送的時間，用於確保同一公告僅發送一次"),
    )
