    ...
    **Permissions:**
    ...
    def get_queryset(self):
        ...
        if not (self.request.user.is_authenticated and 
                ...
                self.request.user.role in [UserRole.SUPER_ADMIN, UserRole.ADMIN]):
    ...
    def publish(self, request, pk=None):
        ...
        if announcement.is_send_email and not announcement.email_sent_at:
            ...
            try:
                ...
                email_sent = True
                logger.info(f"Announcement email sent successfully for announcement {announcement.id}")
            except Exception as e:
                logger.error(f"Failed to send announcement email for announcement {announcement.id}: {str(e)}")
                # 即使發送失敗，也繼續發布公告
        elif announcement.is_send_email and announcement.email_sent_at:
            logger.info(f"Announcement {announcement.id} email already sent at {announcement.email_sent_at}, skipping email send")
    ...
    def _send_announcement_email(self, announcement):
        ...
        clients = User.objects.filter(role=UserRole.CLIENT, is_active=True, email__isnull=False).exclude(email='')
        
        if not clients.exists():
            logger.warning(f"No active clients found to send announcement email for announcement {announcement.id}")
            return
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">新公告通知</h2>
                <p>親愛的會員：</p>
                <p>我們發布了一則新公告：</p>
                ...
                <p style="word-break: break-all; color: #7f8c8d;">
                    <a href="{announcements_url}" style="color: #3498db;">{announcements_url}</a>
                </p>
