...
API_KEY = os.environ.get(
    "API_KEY", "42422ec94d248ec76e47e80b09780fd2b0479c14a79a6c3cebef8a7b14d8424e"
)
TEMPLATE_ID = int(os.environ.get("TEMPLATE_ID", "7969"))
ISSUANCE_SECRET_KEY = os.environ.get(
    "ISSUANCE_SECRET_KEY", r"1L+dRpp\{DV,"
)  # 發證密鑰（從模板或配置中獲取）
...
def get_template(template_id: int, api_key: str) -> Optional[Dict[str, Any]]:
    ...
    params = {"template_id": str(template_id), "api_token": api_key}
            ...
            if business_code == 0:
                ...
                if key_list:
                    ...
                    for key_item in key_list:
                        print(
                            f"  - {key_item.get('key')} ({key_item.get('type')}): {key_item.get('description', '')}"
                        )
                ...
                print_error(
                    f"錯誤訊息: {data.get('message', {}).get('message', 'N/A')}"
                )
                return None
        else:
...
def build_certs_data(
    template_data: Dict[str, Any], certificate_application=None
) -> list:
    """
    ...
    # 構建一個示例證書資料
    cert_data: Dict[str, Any] = {"email": "example@example.com"}  # email 是必填欄位
    ...
    for key_item in key_list:
        ...
        if certificate_application:
            ...
            if "姓名" in description or "name" in description.lower():
                ...
                "醫院" in description
                or "hospital" in description.lower()
                or "就診" in description
            ):
                cert_data[key] = hospital_name
            elif (
                "醫師" in description
                or "doctor" in description.lower()
                or "執行" in description
            ):
                cert_data[key] = surgeon_name
            elif (
                "日期" in description
                or "date" in description.lower()
                or "執行日期" in description
            ):
                cert_data[key] = surgery_date if surgery_date else "2025-12-01"
            elif (
                "序號" in description
                or "number" in description.lower()
                or "certificate no" in description.lower()
                or "認證" in description
            ):
                ...
                if (
                    certificate_application.certificate_data
                    and key in certificate_application.certificate_data
                ):
...
def issue_certificates_to_new_group(
    template_id: int, api_key: str, template_data: Optional[Dict[str, Any]] = None
) -> Optional[int]:
    """
    發證到新的證書群組
    ...
    payload = {
        ...
        "autoNotificationTime": datetime.now(timezone.utc).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        ),  # 當前時間 ISO 8601 格式
        "customEmail": {
            "subject": "證書發放通知",
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if business_code == 0:
                ...
                print_error(
                    f"完整響應: {json.dumps(data, indent=2, ensure_ascii=False)}"
                )
                return None
        else:
            ...
            try:
                ...
                print_error(
                    f"錯誤訊息: {json.dumps(error_data, indent=2, ensure_ascii=False)}"
                )
            except ValueError:
                print_error(f"錯誤訊息: {response.text}")
            return None
...
def main():
    ...
    if ISSUANCE_SECRET_KEY:
        masked_key = (
            "*" * (len(ISSUANCE_SECRET_KEY) - 4) + ISSUANCE_SECRET_KEY[-4:]
            if len(ISSUANCE_SECRET_KEY) > 4
            else "****"
        )
    ...
    certificate_group_id = issue_certificates_to_new_group(
        TEMPLATE_ID, API_KEY, template_data
    )
    ...
    if certificate_group_id:
        ...
        if response.lower() == "y":
            issue_certificates_to_existing_group(
                certificate_group_id, API_KEY, template_data
            )
    else:
