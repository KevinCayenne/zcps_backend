...
class TestBlacklistUtilityFunction:
    ...
    def test_blacklist_user_tokens_blacklists_all_tokens(self, create_user):
        ...
        blacklisted = BlacklistedToken.objects.filter(token__user=user).count()
...
class TestPasswordChangeTokenBlacklisting:
    ...
    def test_password_change_blacklists_tokens_when_setting_enabled(
        self, api_client, create_user, settings
    ):
        ...
        refresh_response = api_client.post(
            "/auth/jwt/refresh/", {"refresh": old_refresh_token}
        )
        assert refresh_response.status_code == 401

    def test_password_change_does_not_blacklist_when_setting_disabled(
        self, api_client, create_user, settings
    ):
