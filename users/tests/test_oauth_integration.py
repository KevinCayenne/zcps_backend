...
class TestOAuthIntegration:
    ...
    def test_jwt_token_generation_for_oauth_user(self):
        ...
        user = User.objects.create_user(
            username="oauthuser", email="oauth@example.com", google_id="google345678"
        )
        user.set_unusable_password()
        user.save()
    ...
    def test_dual_authentication_support(self):
        ...
        user = User.objects.create_user(
            username="dualuser", email="dual@example.com", password="testpass123"
        )
    ...
    def test_oauth_generated_tokens_work_with_protected_endpoints(self):
        ...
        user = User.objects.create_user(
            username="tokentest", email="token@example.com", google_id="google555666"
        )
        user.set_unusable_password()
        user.save()
    ...
    def test_account_linking_updates_existing_user_record(self):
        ...
        user = User.objects.create_user(
            username="update", email="update@example.com", password="oldpass"
        )
    ...
    def test_google_id_uniqueness_prevents_duplicate_oauth_accounts(self):
        ...
        User.objects.create_user(
            username="user1", email="user1@example.com", google_id="google_shared_id"
        )
