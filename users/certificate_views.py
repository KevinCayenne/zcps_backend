...
def get_template(template_id: int) -> Tuple[Optional[Dict[str, Any]], int]:
    ...
    try:
        ...
        response = requests.get(
            external_api_url, params=params, headers=headers, timeout=30  # 30 秒超時
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                return ({"error": "CC 帳號或模板不存在"}, status.HTTP_404_NOT_FOUND)
        ...
        logger.error(
            f"Connection error when calling external API for template_id={template_id}"
        )
        return (
    ...
    try:
        ...
        if response.status_code in [200, 201]:  # 200 或 201 都表示成功
            ...
            try:
                ...
                return ({"error": "發證資料有誤"}, status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return ({"error": "沒有權限執行此操作"}, status.HTTP_403_FORBIDDEN)
        ...
        logger.error(
            "Timeout when calling external API for issue certificates to new group"
        )
        return (
            {"error": "外部 API 請求超時，請稍後再試"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.error(
            "Connection error when calling external API for issue certificates to new group"
        )
        return (
            {"error": "無法連接到外部 API，請檢查網路連接"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def issue_certificates_to_existing_group(
    ...
    try:
        ...
        logger.error(
            "Timeout when calling external API for issue certificates to existing group"
        )
        return (
            {"error": "外部 API 請求超時，請稍後再試"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.error(
            "Connection error when calling external API for issue certificates to existing group"
        )
        return (
            {"error": "無法連接到外部 API，請檢查網路連接"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        **注意事項：**
        ...
        request=inline_serializer(
            ...
            fields={
                ...
                "certRecordGroupId": serializers.IntegerField(
                    required=True, help_text="證書群組 ID"
                ),
            },
        ),
        examples=[
...
def build_certs_data_from_template(
    ...
    if certificate_application:
        ...
        if not surgeon_name and certificate_application.certificate_data:
            surgeon_name = certificate_application.certificate_data.get(
                "tx-103", ""
            ) or certificate_application.certificate_data.get("surgeon_name", "")
        ...
        if certificate_application.surgery_date:
            ...
            date_value = certificate_application.certificate_data.get(
                "tx-104", ""
            ) or certificate_application.certificate_data.get("surgery_date", "")
            if date_value:
        ...
        if not certificate_application.certificate_number:
            certificate_application.certificate_number = (
                certificate_application.generate_certificate_number()
            )
        ...
        cert_data = {
            "email": (
                user_data.get("email")
                if user_data
                else user.email or "example@example.com"
            )
        }
    else:
        ...
        cert_data = {
            "email": (
                user_data.get("email", "example@example.com")
                if user_data
...
class IssueCertificatesWithTemplateView(APIView):
        ...
        request=inline_serializer(
            ...
            fields={
                "token": serializers.CharField(
                    required=True, help_text="驗證 token（從驗證 email 中獲取）"
                ),
                "name": serializers.CharField(required=False, help_text="群組名稱"),
                "isDownloadButtonEnabled": serializers.BooleanField(
                    required=False, help_text="是否啟用下載按鈕"
                ...
                "certificateData": serializers.DictField(
                    required=False,
                    help_text=(
                        "證書資料。必須包含 email 欄位，可選包含 tx- 開頭的模板欄位值。"
                        "未提供的欄位會使用默認值。如果未提供，將使用申請時提交的資料。"
                    ),
                ),
            },
        ),
            ...
            OpenApiExample(
                ...
                value={
                    ...
                    "message": {
                        "executionTime": "2025-12-01T12:04:45.101Z",
                        "message": (
                            "Successfully built a new certificate group and started the process "
                            "of producing certificates."
                        ),
                    },
                    "content": {"id": 13107},
                },
    ...
    def post(self, request):
        ...
        if not token:
            return Response(
                {"error": "缺少驗證 token"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "無效的驗證 token"}, status=status.HTTP_400_BAD_REQUEST
            )

        # 檢查 token 是否有效
        if not application.is_token_valid():
            return Response(
                {"error": "Token 已過期或已被使用"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        user_certificate_data = request_data.get(
        ...
        issue_request_data = {
            ...
            "isDownloadButtonEnabled": request_data.get(
                "isDownloadButtonEnabled", True
            ),
            "skipSendingNotification": request_data.get(
                "skipSendingNotification", True
            ),
        ...
        if "autoNotificationTime" in request_data:
            issue_request_data["autoNotificationTime"] = request_data[
                "autoNotificationTime"
            ]
        else:
            # 如果沒有提供，使用當前時間
            issue_request_data["autoNotificationTime"] = datetime.now(
                timezone.utc
...
def get_certificate(
    cert_id: Optional[int] = None, cert_hash: Optional[str] = None
) -> Tuple[Optional[Dict[str, Any]], int]:
    """
    從外部 API 獲取證書詳細資料。
    ...
    # 構建外部 API URL
    external_api_url = (
        f"{external_api_base_url}/openapi/v1/cert-record-groups/get-certificates"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                return ({"error": "請求參數錯誤"}, status.HTTP_400_BAD_REQUEST)
            ...
            try:
                ...
                return ({"error": "沒有權限訪問此證書"}, status.HTTP_403_FORBIDDEN)
            ...
            try:
                ...
                return ({"error": "證書不存在"}, status.HTTP_404_NOT_FOUND)
        ...
        logger.error(
            f"Timeout when calling external API for certificate id={cert_id}, hash={cert_hash}"
        )
        return (
            {"error": "外部 API 請求超時，請稍後再試"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        logger.error(
            f"Connection error when calling external API for certificate id={cert_id}, hash={cert_hash}"
        )
        return (
            {"error": "無法連接到外部 API，請檢查網路連接"},
            status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_pdf_url(pdf_id: str) -> Tuple[dict[str, Any] | str, int]:
    """
    ...
    Returns:
        Tuple[dict[str, Any] | str, int]: (PDF URL 或錯誤訊息, HTTP 狀態碼)
    ...
    """
    if not pdf_id:
        return ({"error": "pdf_id 是必填參數"}, status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        if response.status_code in [301, 302, 307, 308]:
            ...
            try:
                ...
                return ({"error": "沒有權限訪問此 PDF"}, status.HTTP_403_FORBIDDEN)
            ...
            try:
                ...
                return ({"error": "PDF 檔案不存在"}, status.HTTP_404_NOT_FOUND)
