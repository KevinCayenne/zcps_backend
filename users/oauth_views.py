        ...
        try:
            ...
            if "error" in request.GET:
                ...
                if error == "access_denied":
                    ...
                    return HttpResponseRedirect(
                        build_error_redirect_url("oauth_error", f"OAuth error: {error}")
                    )
            ...
            if not code:
                ...
                return HttpResponseRedirect(
                    build_error_redirect_url(
                        "invalid_request", "No authorization code provided."
                    )
                )
            ...
            if settings.TWOFACTOR_ENFORCE_FOR_ALL_USERS and not user.is_2fa_enabled:
                ...
                logger.info(
                    f"OAuth user {user.email} requires 2FA setup (global enforcement)"
                )
                ...
                params = urlencode(
                    {"temp_token": temp_token, "requires_2fa_setup": "true"}
                )
            ...
            return HttpResponseRedirect(
                build_success_redirect_url(access_token_jwt, refresh_token_jwt)
            )
