...
class UserSerializer(serializers.ModelSerializer):
    ...
    clinic_permissions = serializers.SerializerMethodField(
        read_only=True, help_text="該用戶的診所權限列表（只讀）"
    )
    ...
    def get_clinic_permissions(self, obj):
        ...
        permissions = ClinicUserPermission.objects.filter(user=obj).select_related(
            "clinic"
        )
        ...
        return [
            {
                ...
                "clinic_number": (
                    perm.clinic.number if hasattr(perm.clinic, "number") else None
                ),
    ...
    def create(self, validated_data):
        ...
        username = (
            validated_data.get("username", "").strip()
        ...
        if username and User.objects.filter(username=username).exists():
            raise serializers.ValidationError({"username": ["此使用者帳號已被使用。"]})
...
class SimpleUserCreateSerializer(serializers.ModelSerializer):
    ...
    surgery_date = serializers.DateField(
        required=True, write_only=True, help_text="手術執行日期"
    )
    ...
    def create(self, validated_data):
        ...
        getattr(self, "clinic_id", None)
        getattr(self, "surgery_date", None)
        getattr(self, "surgeon_name", None)
...
class UserCreateSerializer(DjoserUserCreateSerializer):
    ...
    surgery_date = serializers.DateField(
        required=False, allow_null=True, help_text="手術執行日期（可選）"
    )
