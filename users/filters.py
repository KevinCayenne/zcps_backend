...
class UserFilterSet(django_filters.FilterSet):
    ...
    last_login__gte = django_filters.DateTimeFilter(
        field_name="last_login", lookup_expr="gte"
    )
    last_login__lte = django_filters.DateTimeFilter(
    ...
    clinic = django_filters.NumberFilter(
        method="filter_by_clinic", help_text="診所 ID（用於篩選擁有該診所權限的用戶）"
    )
    clinic_id = django_filters.NumberFilter(
        method="filter_by_clinic", help_text="診所 ID（用於篩選擁有該診所權限的用戶）"
    )
    ...
    def filter_by_clinic(self, queryset, name, value):
        ...
        try:
            ...
            # 找到所有擁有該診所權限的用戶 ID
            user_ids = (
                ClinicUserPermission.objects.filter(clinic_id=clinic_id)
