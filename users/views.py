                ...
                response=inline_serializer(
                    ...
                    fields={
                        ...
                        "available": serializers.BooleanField(
                            help_text="email 是否可用（true=可用，false=已被使用）"
                        ),
...
class LogoutView(APIView):
    ...
    @extend_schema(
        ...
        **Security:**
        ...
        responses={
            204: OpenApiResponse(
                description="Logout successful, token blacklisted (no content returned)"
            ),
            400: OpenApiResponse(
...
class CustomUserViewSet(DjoserUserViewSet):
    ...
    @extend_schema(
        ...
        **After Registration:**
        ...
        examples=[
            ...
            OpenApiExample(
                "Success Response",
                value={"id": 1, "email": "newuser@example.com", "username": "newuser"},
                response_only=True,
                status_codes=["201"],
            ),
        ],
        responses={
            201: OpenApiResponse(
                description="User created successfully. Check email for activation if required."
            ),
            400: OpenApiResponse(
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            # 檢查驗證狀態（預設僅 Email，需要手機驗證時再要求）
            email = request.data.get("email", "").strip()
            phone_number = request.data.get("phone_number", "").strip()
            verification_mode = (
                request.data.get("verification_mode", "email_only") or "email_only"
            )
            verification_mode = str(verification_mode).strip().lower()

            if verification_mode not in {"email_only", "email_and_phone"}:
                return Response(
                    {"error": "verification_mode 僅支援 email_only 或 email_and_phone"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if has_only_simple_fields:
                ...
                serializer = SimpleUserCreateSerializer(data=request_data)
            else:
            ...
            try:
                    ...
                    if certificate_fields.get("clinic_id"):
                        ...
                        try:
                            clinic = Clinic.objects.get(
                                id=certificate_fields["clinic_id"]
                            )
                        except Clinic.DoesNotExist:
                            raise ValueError(
                                f'指定的診所不存在 (ID: {certificate_fields.get("clinic_id")})'
                            )

                        # 檢查診所是否有 email
                        if not clinic.email:
                            raise ValueError(
                                "診所未設置電子郵件地址，無法發送驗證 email"
                            )
                        ...
                        if certificate_fields.get("consultation_clinic_id"):
                            try:
                                ...
                                raise ValueError(
                                    f'指定的諮詢診所不存在 (ID: {certificate_fields.get("consultation_clinic_id")})'
                                )
                        ...
                        if certificate_fields.get("surgeon_name"):
                            certificate_data["surgeon_name"] = certificate_fields[
                                "surgeon_name"
                            ]
                        if certificate_fields.get("surgery_date"):
                            certificate_data["surgery_date"] = (
                                certificate_fields["surgery_date"].strftime("%Y-%m-%d")
                                if hasattr(
                                    certificate_fields["surgery_date"], "strftime"
                                )
                                else str(certificate_fields["surgery_date"])
                            )
                ...
                logger.error(
                    f"Failed to create user due to database integrity error: {e}",
                    exc_info=True,
                )
                error_message = str(e)
                lowered_message = error_message.lower()
                if (
                    "phone_number" in lowered_message
                    or "users_user_phone_number" in lowered_message
                ):
                    error_message = "此手機號碼已被使用，請使用其他手機號碼註冊"
                elif (
                    "email" in lowered_message or "users_user_email" in lowered_message
                ):
                    error_message = "此電子郵件已被使用，請使用其他電子郵件註冊"
                return Response(
                    {"error": "創建用戶失敗", "message": error_message},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            except DjangoValidationError as e:
                logger.error(
                    f"Failed to create user due to validation error: {e}", exc_info=True
                )
                return Response(
                    {
                        "error": "創建用戶失敗",
                ...
                return Response(
                    {"error": "創建失敗", "message": str(e)},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            except Exception as e:
                logger.error(
                    f"Failed to create user or certificate application: {e}",
                    exc_info=True,
                )
                return Response(
                    {"error": "創建失敗", "message": "系統錯誤，請稍後再試"},
            ...
            if not user:
                return Response(
                    {"error": "創建失敗", "message": "用戶創建失敗，請稍後再試"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            ...
            if certificate_fields.get("clinic_id") and application and token:
                ...
                try:
                    ...
                    if clinic.email:
                        try:
                            self._send_certificate_verification_email(
                                application, token
                            )
                        except Exception as e:
                            logger.error(
                    ...
                    logger.error(
                        f"Failed to send certificate verification email: {e}",
                        exc_info=True,
                    )
            ...
            try:
                ...
                # 創建並發送註冊成功郵件
                registration_email = RegistrationSuccessEmail(
                    request,
                    {
                        "user": user,
                        "site_name": site_name,
                ...
                logger.info(
                    f"Registration success email sent to {user.email} for user {user.id}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to send registration success email for user {user.id}: {e}",
                    exc_info=True,
                )
                # 不阻止響應，因為用戶已經創建成功
            ...
            return Response(
                {"message": "用戶創建成功", "data": serializer.data},
    ...
    @extend_schema(
        ...
        **Security:**
        ...
        examples=[
            OpenApiExample(
                "Update User Request",
                value={"email": "updated@example.com", "username": "updateduser"},
                request_only=True,
            ),
        ],
        responses={
        ...
        request=inline_serializer(
            ...
            fields={
                "email": serializers.EmailField(
                    required=False, help_text="Email address (optional)"
                ),
                "username": serializers.CharField(
                    required=False, help_text="Username (optional)"
                ),
            },
        ),
        examples=[
        ...
        **Difference from PATCH:**
        ...
        examples=[
            OpenApiExample(
                ...
                value={"email": "newemail@example.com", "username": "newusername"},
                request_only=True,
            ),
        ],
        responses={
    ...
    def _send_certificate_verification_email(self, application, token):
        ...
        surgeon_name = application.surgeon_name or application.certificate_data.get(
            "surgeon_name", "未提供"
        )
        surgery_date = (
            application.surgery_date.strftime("%Y-%m-%d")
            if application.surgery_date
            else application.certificate_data.get("surgery_date", "未提供")
        )
    ...
    @extend_schema(
        ...
        **Common Errors:**
        ...
        examples=[
            OpenApiExample(
                "Activation Request",
                value={"uid": "MQ", "token": "abc123-def456-ghi789"},
                request_only=True,
            ),
        ],
        responses={
            204: OpenApiResponse(
                description="Account activated successfully (no content returned)"
            ),
            400: OpenApiResponse(
    ...
    @extend_schema(
        ...
        **After Receiving Email:**
        ...
        request=inline_serializer(
            ...
            fields={
                "email": serializers.EmailField(
                    help_text="Email address of account to activate"
                ),
            },
        ),
        examples=[
        ...
        responses={
            204: OpenApiResponse(
                description="Activation email sent successfully (no content returned)"
            ),
            400: OpenApiResponse(
    ...
    @extend_schema(
        ...
        **Security:**
        ...
        request=inline_serializer(
            ...
            fields={
                "email": serializers.EmailField(
                    help_text="Email address of account to reset"
                ),
            },
        ),
        examples=[
    ...
    @extend_schema(
        ...
        **After Success:**
        ...
        responses={
            204: OpenApiResponse(
                description="Password reset successful. All JWT tokens blacklisted. Login required."
            ),
            400: OpenApiResponse(
                ...
                response=inline_serializer(
                    ...
                    fields={
                        ...
                        "token": serializers.ListField(
                            help_text="Token validation errors"
                        ),
    ...
    @extend_schema(
        ...
        **Security:**
        ...
        responses={
            204: OpenApiResponse(
                description=(
                    "Password changed successfully. Check BLACKLIST_TOKENS_ON_PASSWORD_CHANGE "
                    "setting to see if re-login required."
                )
            ),
            400: OpenApiResponse(
                description="Bad Request - Validation errors",
                response=inline_serializer(
                    ...
                    fields={
                        "current_password": serializers.ListField(
                            help_text="Current password errors"
                        ),
                        "new_password": serializers.ListField(
    ...
    def set_password(self, request, *args, **kwargs):
        ...
        if response.status_code == status.HTTP_204_NO_CONTENT and getattr(
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def _update_clinic_permissions(self, user, clinic_ids):
        ...
        current_clinic_ids = set(
            current_permissions.values_list("clinic_id", flat=True)
        )
        ...
        for clinic_id in to_add:
            ...
            # 檢查是否已存在（避免重複）
            if not ClinicUserPermission.objects.filter(
        ...
        if to_remove:
            ClinicUserPermission.objects.filter(
                user=user, clinic_id__in=to_remove
            ).delete()
...
class ClientUserOuterViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.id != request.user.id:
            return Response(
                {"detail": "您只能更新自己的資料。"}, status=status.HTTP_403_FORBIDDEN
            )
        return super().update(request, *args, **kwargs)
