...
class ActionLogFilter(filters.FilterSet):
    ...
    model_name = filters.CharFilter(method="filter_by_model_name_object_id", required=False)
    # 獨立的 model_name 過濾器，用於單獨使用
    model_name_only = filters.CharFilter(field_name='model_name', lookup_expr='exact', required=False)
    # object_id 過濾器，但只在與 model_name_only 組合使用時才生效
    object_id = filters.CharFilter(field_name='object_id', lookup_expr='exact', required=False)

    class Meta:
        model = ActionLog
        fields = ["user", "action", "model_name", "model_name_only", "object_id", "timestamp", "changes"]
    ...
    def filter_by_model_name_object_id(self, queryset, name, value):
        ...
        for pair in pairs:
            ...
            try:
                ...
                if model_name == "ClinicUserPermission" and ClinicUserPermission is not None:
                    try:
                        user_id = int(object_id)
                        user_id_str = str(user_id)
                        
                        # 找到所有 user_id 等於該值的 ClinicUserPermission 實例
                        permission_ids = list(ClinicUserPermission.objects.filter(
                            user_id=user_id
                        ...
                        clinic_permission_conditions |= Q(
                            model_name=model_name,
                            changes__icontains=user_id_str
                        )
