...
class ClinicFilterSet(django_filters.FilterSet):
    ...
    phone = django_filters.CharFilter(
        lookup_expr="icontains", help_text="電話（部分匹配，不區分大小寫）"
    )
    email = django_filters.CharFilter(
        lookup_expr="icontains", help_text="電子郵件（部分匹配，不區分大小寫）"
    )
    website = django_filters.CharFilter(
        lookup_expr="icontains", help_text="網站（部分匹配，不區分大小寫）"
    )

    # Date fields - support range filtering
    create_time = django_filters.DateTimeFilter()
    create_time__gte = django_filters.DateTimeFilter(
        field_name="create_time", lookup_expr="gte", help_text="建立時間（大於等於）"
    )
    create_time__lte = django_filters.DateTimeFilter(
        field_name="create_time", lookup_expr="lte", help_text="建立時間（小於等於）"
    )

    update_time = django_filters.DateTimeFilter()
    update_time__gte = django_filters.DateTimeFilter(
        field_name="update_time", lookup_expr="gte", help_text="更新時間（大於等於）"
    )
    update_time__lte = django_filters.DateTimeFilter(
        field_name="update_time", lookup_expr="lte", help_text="更新時間（小於等於）"
    )
...
class CertificateApplicationFilterSet(django_filters.FilterSet):
    ...
    status = django_filters.ChoiceFilter(
        choices=CertificateApplicationStatus.CHOICES, help_text="申請狀態"
    )

    # Text fields with icontains search
    surgeon_name = django_filters.CharFilter(
        lookup_expr="icontains", help_text="手術醫師姓名（部分匹配，不區分大小寫）"
    )
    consultant_name = django_filters.CharFilter(
        lookup_expr="icontains", help_text="諮詢師姓名（部分匹配，不區分大小寫）"
    )
    ...
    verified_at__gte = django_filters.DateTimeFilter(
        field_name="verified_at", lookup_expr="gte", help_text="驗證時間（大於等於）"
    )
    verified_at__lte = django_filters.DateTimeFilter(
        field_name="verified_at", lookup_expr="lte", help_text="驗證時間（小於等於）"
    )

    issued_at = django_filters.DateTimeFilter()
    issued_at__gte = django_filters.DateTimeFilter(
        field_name="issued_at", lookup_expr="gte", help_text="發證時間（大於等於）"
    )
    issued_at__lte = django_filters.DateTimeFilter(
        field_name="issued_at", lookup_expr="lte", help_text="發證時間（小於等於）"
    )
