...
class CertificateApplicationViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = CertificateApplication.objects.select_related(
    ...
    def create(self, request, *args, **kwargs):
        ...
        consultation_clinic = Clinic.objects.get(id=consultation_clinic_id) if consultation_clinic_id else None
        ...
        try:
            with transaction.atomic():
                ...
                application = CertificateApplication(
                    ...
                    status=serializer.validated_data.get('status', CertificateApplicationStatus.PENDING),
                    create_user=request.user if request.user.is_authenticated else None
                )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if 'consultation_clinic_id' in serializer.validated_data:
            consultation_clinic_id = serializer.validated_data.pop('consultation_clinic_id')
            consultation_clinic = Clinic.objects.get(id=consultation_clinic_id) if consultation_clinic_id else None
    ...
    def toggle_status(self, request, pk=None):
        ...
        if current_status not in [CertificateApplicationStatus.PENDING, CertificateApplicationStatus.CANCELLED]:
            return Response(
                {
    ...
    @action(detail=True, methods=['post'], url_path='resend-verification-email', url_name='resend-verification-email', permission_classes=[IsAuthenticated, IsStaffRolePermission])
    ...
    def _send_verification_email(self, application, token):
        ...
        <body>
            ...
            <ul>
                <li><strong>申請人姓名：</strong>{applicant_name or '未提供'}</li>
                <li><strong>申請人 Email：</strong>{applicant_email or '未提供'}</li>
                <li><strong>申請人電話：</strong>{applicant_phone or '未提供'}</li>
                <li><strong>手術醫師：</strong>{surgeon_name or '未提供'}</li>
                <li><strong>手術日期：</strong>{surgery_date or '未提供'}</li>
                <li><strong>申請時間：</strong>{application.create_time.strftime('%Y-%m-%d %H:%M:%S')}</li>
            </ul>
            <p>請點擊以下連結確認並驗證此申請：</p>
            <p><a href="{verification_url}" style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">確認證書申請</a></p>
            <p>或複製以下連結到瀏覽器：</p>
            <p>{verification_url}</p>
            <p>此連結將在 {application.token_expires_at.strftime('%Y-%m-%d %H:%M:%S') if application.token_expires_at else '7天'} 後過期。</p>
...
class IssueCertificateView(APIView):
            ...
            200: OpenApiResponse(
                ...
                response=inline_serializer(
                    ...
                    fields={
                        'success': serializers.BooleanField(help_text='是否成功'),
                        'application_id': serializers.IntegerField(help_text='申請 ID'),
                        'certificate_group_id': serializers.IntegerField(help_text='證書群組 ID'),
                        'certificate_hash': serializers.CharField(required=False, help_text='證書 hash 值'),
                        'status': serializers.CharField(help_text='申請狀態'),
                        'message': serializers.CharField(help_text='成功訊息'),
                    }
                )
    ...
    def post(self, request):
        ...
        user_certificate_data = request.data.get('certificateData', application.certificate_data)
        ...
        if 'email' not in user_certificate_data or not user_certificate_data.get('email'):
            return Response(
        ...
        if 'autoNotificationTime' in request.data:
            issue_request_data['autoNotificationTime'] = request.data['autoNotificationTime']
        else:
            # 如果沒有提供，使用當前時間
            issue_request_data['autoNotificationTime'] = datetime.now(tz.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
        ...
        if status_code not in [status.HTTP_200_OK, status.HTTP_201_CREATED]:
            return Response(
                {'error': '發證失敗', 'details': response_data},
                status=status_code
            )
