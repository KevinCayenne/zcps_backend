...
class Doctor(BaseModel):
    ...
    # 基本資訊
    name = models.CharField(
        max_length=255, verbose_name=_("醫生姓名"), help_text=_("醫生姓名")
    )
    email = models.EmailField(
        max_length=255,
        verbose_name=_("電子郵件"),
    ...
    is_active = models.BooleanField(
        default=True, verbose_name=_("是否啟用"), help_text=_("醫生是否仍在該診所執業")
    )

    # 備註
    notes = models.TextField(
        verbose_name=_("備註"), blank=True, null=True, help_text=_("其他備註資訊")
    )
...
class CertificateApplication(BaseModel):
    ...
    token_expires_at = models.DateTimeField(
        verbose_name=_("Token 過期時間"), help_text=_("驗證 token 的過期時間")
    )
    ...
    def generate_certificate_number(self):
        ...
        # 確保唯一性（如果已存在，重新生成）
        while CertificateApplication.objects.filter(
    ...
    def get_applicant_name(self):
        ...
        if self.user:
            return (
                f"{self.user.last_name or ''}{self.user.first_name or ''}".strip()
